	 ...
	 @Override
	    ...
	    protected void placeCurrentPositionMarker(double lat, double lon) {
	    	if(currentPosition == null) {
	    		...
	    		// Create a new radius which shows roughly where the available chatrooms are
	    ...
	    private Marker createChatroom(Chatroom room) {
	    	...
	    	if( 1 == new Random().nextInt()) { //needs to change available
	    		m = BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN);
	    	} else {// unavailabe
	    		m = BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED);
	    	}
